...
import { Result, success, failure, StandardError, ErrorCategory } from '@party/types';
...
export class FrontendErrorHandler {
   ...
   */
  handleBackendError(errorResponse: unknown, context: string): ErrorHandlingResult<never> {
    ...
    if (errorResponse && typeof errorResponse === 'object') {
      ...
      const frontendError: FrontendError = {
        ...
        message: (response.error as string) || (response.message as string) || 'An unknown error occurred',
  ...
  shouldRetry(error: FrontendError): boolean {
    const strategy = this.config.retryStrategies.get(error.code) || this.config.defaultRetryStrategy;
    return strategy.retryable && this.getErrorCount(error.code) < strategy.maxRetries;
  }
  ...
  getRetryStrategy(error: FrontendError): ErrorRecoveryStrategy {
    return this.config.retryStrategies.get(error.code) || this.config.defaultRetryStrategy;
  }
  ...
  private toFrontendError(error: unknown, context: string): FrontendError {
    ...
    if (error && typeof error === 'object' && 'status' in error) {
      const networkError = error as { status: number; statusText?: string; url?: string };
      return {
        code: 'NETWORK_ERROR',
...
export const handleError = (error: unknown, context: string) => errorHandler.handleError(error, context);
export const handleAsync = <T>(operation: () => Promise<T>, context: string) => errorHandler.handleAsync(operation, context);
export const handleBackendError = (errorResponse: unknown, context: string) => errorHandler.handleBackendError(errorResponse, context);
