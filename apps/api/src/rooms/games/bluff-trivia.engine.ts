...
export class BluffTriviaEngine
  implements GameEngine<BluffTriviaState, BluffTriviaAction, BluffTriviaEvent>
{
  private phases: GamePhase[] = [
    { name: PHASE_NAMES.LOBBY, duration: 0, allowedActions: ['join', 'start'] },
    {
      name: PHASE_NAMES.PROMPT,
      duration: GAME_PHASE_DURATIONS.PROMPT,
      allowedActions: ['submitAnswer'],
    },
    {
      name: PHASE_NAMES.CHOOSE,
      duration: GAME_PHASE_DURATIONS.CHOOSE,
      allowedActions: ['submitVote'],
    },
    {
      name: PHASE_NAMES.SCORING,
      duration: GAME_PHASE_DURATIONS.SCORING,
      allowedActions: [],
    },
  ...
  processAction(
    state: BluffTriviaState,
    action: BluffTriviaAction,
  ): GameResult<BluffTriviaState, BluffTriviaEvent> {
  ...
  getValidActions(
    ...
    if (
      currentPhase.allowedActions.includes('start') &&
    ...
    if (
      currentPhase.allowedActions.includes('submitAnswer') &&
      state.phase === PHASE_NAMES.PROMPT
    ) {
      actions.push({ type: 'submitAnswer', playerId, data: {} });
    }

    if (
      currentPhase.allowedActions.includes('submitVote') &&
      state.phase === PHASE_NAMES.CHOOSE
    ) {
  ...
  private handleStart(
    ...
    return {
      ...
      events: [
        ...
        {
          type: 'prompt',
          data: { question: newState.currentRound!.prompt },
          target: 'all',
        },
      ],
      isValid: true,
  ...
  private handleSubmitAnswer(
    ...
    const isCorrectAnswer =
      text.toLowerCase() === state.currentRound.answer.toLowerCase();
    ...
    if (isCorrectAnswer) {
      ...
      if (player) {
        ...
        const newCorrectAnswerPlayers = new Set(
          state.currentRound.correctAnswerPlayers,
        );
  ...
  private handleSubmitVote(
    ...
    const playerBluff = state.currentRound.bluffs.find(
      (b) => b.by === action.playerId,
    );
  ...
  private generateChoices(
    round: BluffTriviaRound,
  ): Array<{ id: string; text: string }> {
    // Start with the truth - normalize text to prevent capitalization hints
    const choices = [
      { id: TRUE(round.promptId), text: this.normalizeText(round.answer) },
    ];
