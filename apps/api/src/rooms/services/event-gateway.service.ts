...
export class EventGatewayService {
  ...
  async broadcastEvents(
    ...
    try {
      ...
      const errorResponse = this.errorHandler.handleError(error, 'event-broadcast');
      this.logger.error(`Error broadcasting events to room ${roomCode}:`, errorResponse);
   ...
   */
  async broadcastRoomUpdate(roomCode: string, roomData: any): Promise<Result<void, any>> {
    try {
      ...
      const errorResponse = this.errorHandler.handleError(error, 'room-update-broadcast');
      this.logger.error(`Error broadcasting room update to room ${roomCode}:`, errorResponse);
      return failure(errorResponse);
    }
  }
  ...
  async handleGameEvents(
    ...
    try {
      ...
      this.logger.log(`Processed ${events.length} game events for room ${roomCode}`);
      return success(undefined);
    } catch (error) {
      const errorResponse = this.errorHandler.handleError(error, 'game-events-handling');
      this.logger.error(`Error handling game events for room ${roomCode}:`, errorResponse);
      return failure(errorResponse);
    }
  }
}
