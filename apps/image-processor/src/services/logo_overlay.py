...
class LogoOverlayService:
    ...
    async def overlay_roster_on_tshirt_back(
        ...
        try:
            ...
            start_x = tshirt_width // 2 - 100  # Center horizontally
            start_y = tshirt_height // 2 - 50  # Center vertically
            line_height = 60
            current_y = start_y
    ...
    async def create_banner(
        ...
        try:
            ...
            # Add team name
            team_x = int(banner_img.width * 0.3)
            team_y = int(banner_img.height * 0.1)
            draw.text((team_x, team_y), team_name, fill=(0, 0, 0), font=title_font)
            
            # Add roster
            roster_x = int(banner_img.width * 0.3)
            roster_y = int(banner_img.height * 0.6)
            roster_text = " | ".join([f"{p['number']} {p['name']}" for p in players])
    ...
    def _calculate_banner_logo_size(self, banner_img: Image.Image, logo_img: Image.Image) -> tuple:
        ...
        # Calculate target size (20% of banner width)
    ...
    def _calculate_banner_logo_position(self, banner_img: Image.Image, logo_width: int, logo_height: int) -> tuple:
        ...
        # Position logo in top-left area
        x = int(banner_width * 0.05)
